...
tableau_output <- F    # if input files for Tableau should be created
...
#Boxplot fuer PASS
writexl::write_xlsx(spec_bauteil, './output/boxplot_C309_values.xlsx')
...
#         coord_cartesian(xlim = c(-2, 2)) +
         ...
         coord_flip()
if (plots_to_disc) ggsave ("./output/boxplot_C1121_PASS.png")
...
ggplot(data = spec_bauteil[spec_bauteil$SERGEBNIS=="FAIL",], mapping = aes(y = DMW_norm)) +  
         geom_boxplot(mapping = aes(y = DMW_norm)) +
#         coord_cartesian(xlim = c(-2, 2)) +
         ggtitle (paste("distribution for", spec_SMMNR, "C1121 on A5E02758387 - FAIL cycles")) +
         coord_flip()
if (plots_to_disc) ggsave ("./output/boxplot_C1121_PAIL.png")
...
if (!Adaptive_Lasso) {
  ...
  cv.lasso_mod <- cv.glmnet(as.matrix(select(train_n, -c(NTHID, SFILENAME, DTINSERT, SID, SARTIKELNR, DTDATUM, SPLATZNR, SERGEBNIS, BEFUND_clean, pseudo, firstpass_scen, secondpass_scen, firstsecondpass_scen, xpass))), train_n$firstsecondpass_scen, alpha=1, parallel=TRUE, standardize=FALSE, type.measure='auc')
}
if (Adaptive_Lasso) {
  ## Ridge Regression to create the Adaptive Weights Vector
  cv.ridge <- cv.glmnet(as.matrix(select(train_n, -c(NTHID, SFILENAME, DTINSERT, SID, SARTIKELNR, DTDATUM, SPLATZNR, SERGEBNIS, BEFUND_clean, pseudo, firstpass_scen, secondpass_scen, firstsecondpass_scen, xpass))), train_n$firstsecondpass_scen,  alpha=0, parallel=TRUE, standardize=TRUE)
  w3 <- 1/abs(matrix(coef(cv.ridge, s=cv.ridge$lambda.min)[, 1][2:(ncol(select(train_n, -c(NTHID, SFILENAME, DTINSERT, SID, SARTIKELNR, DTDATUM, SPLATZNR, SERGEBNIS, BEFUND_clean, pseudo, firstpass_scen, secondpass_scen, firstsecondpass_scen, xpass)))+1)] ))^1 ## Using gamma = 1
  w3[w3[,1] == Inf] <- 999999999 ## Replacing values estimated as Infinite for 999999999

  ## Adaptive Lasso
  cv.lasso_mod <- cv.glmnet(as.matrix(select(train_n, -c(NTHID, SFILENAME, DTINSERT, SID, SARTIKELNR, DTDATUM, SPLATZNR, SERGEBNIS, BEFUND_clean, pseudo, firstpass_scen, secondpass_scen, firstsecondpass_scen, xpass))), train_n$firstsecondpass_scen, alpha=1, parallel=TRUE, standardize=TRUE, type.measure='auc', penalty.factor=w3)  
}

cv.lasso_mod$lambda.min
...
top_Lasso_coeff[1:15,] # top 15 positive coefficients
top_Lasso_coeff[(nrow(top_Lasso_coeff)-15):nrow(top_Lasso_coeff),]  # top 15 negative coefficients
```
...
Feature Selection     | Seed     | Oversampling | Cutoff | Accuracy
----------------------|----------|--------------|--------|--------------------------
ll variables          | 123      |   no         | 0.50   | 98,71% (4 false positive) 
standardization=TRUE  |          |              |        |      
                      |          |              |        |  
all variables         | 123456789|   no         | 0.60   | 98,92% (2 false positive)
standardization=TRUE  |          |              |        |      
...
Adaptive Lasso reduces the number of coeffiencts to about 190 whereas Lasso comes up wiht about 205 coeffiencts 
                      
```{r, echo = FALSE, message=FALSE, warning=FALSE}
library(e1071)
# Evaluation with test data set
yhat <- predict(cv.lasso_mod, as.matrix(select(test_n, -c(NTHID, SFILENAME, DTINSERT, SID, SARTIKELNR, DTDATUM, SPLATZNR, SERGEBNIS, BEFUND_clean, pseudo, firstpass_scen, secondpass_scen, firstsecondpass_scen, xpass))), type = "class", s = "lambda.min")
yhat_prob <- fn_norm_zo(yhat)        # normalize predicte values
yhat_label <- as.factor(prob_to_class(yhat_prob, 0.60))

# Confusion matrix
ytrue <- test_n$firstsecondpass_scen
caret::confusionMatrix(yhat_label, as.factor(ytrue), positive="1")

# Lift / ROC Chart for test data
par(mfrow=c(1,2))
Lift_Chart (as.vector(yhat_prob), ytrue, "Lift Logistic Regression", "black")
ROC_Chart (yhat_prob, ytrue, "ROC Logistic Regression")
```
location of predictors on the board (visual clustering)
