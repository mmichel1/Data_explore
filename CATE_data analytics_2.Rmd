...
tableau_output <- T    # if input files for Tableau should be created
...
if (basic_tranform) {
  rawframe <- rawframe %>% filter(SEINHEIT != 'OE') %>% filter((SMMNR!='IsolationTest') & (SMMNR!='ConnectionTest')) 
  
  #filter out 11 duplicated A5E02758387 boards with file extension ".Erg.erg"
  rawframe <- rawframe %>% filter(substring(SFILENAME, nchar(SFILENAME)-7, nchar(SFILENAME)) != ".Erg.erg")
  #furthermore A5E02758387 6 passed boards were tested twice:
  #("T-H66308597", "T-H76109577", "T-H76190294", "T-J86287024", "T-J86414519", "T-J86353546")
}

#Board T-J86414591 shows > 400 violations with strange DMW values --> eliminate for analytics modelling
if (!tableau_output) rawframe <- rawframe %>% filter (SID != 'T-J86414591') 
...
Error_mapping <- data.frame(BEFUND = c("B000", "B100", "B317", "B319", "B400", "V110", "V120", "V130", "V132", "V133", "V140", "V210", "V230"),
  BEFUND_clean = c("Pseudo Fehler", "Offene Loetstelle", "Falsches Bauteil", "Kurzschluss", "Offene Loetstelle", "Offene Loetstelle", "Falsches Bauteil", "Falsches Bauteil", "Offene Loetstelle", "Offene Loetstelle", "Offene Loetstelle", "Offene Loetstelle", "Kurzschluss"))
...
# loop wird ab 3 Mio. Interationen äußerst langsam, wenn basic_transform = F!
...
count_boards <- SID_table %>% group_by(SARTIKELNR) %>% summarize(count=n()) %>% left_join(count_pseudo, by ="SARTIKELNR" )
print(count_boards)
...
# Testquery for SID # received from St. Kraus 7.9
#view <- SFILENR_table %>% filter (SID %in% c("T-J86414591", "T-J86258604", "T-J86286945", "T-J86286978", "T-J86324036", "T-J86324101", "T-J86353536", "T-J86353555", "T-J86353569", "T-J86353595", "T-J86353596", "T-J86353611", "T-J86353615", "T-J86353636", "T-J86353640", "T-J86414517", "T-J86414521", "T-J86414549", "T-J86414557", "T-J86414571", "T-J86414575", "T-J86414580", "T-J86414586", "T-J86414587", "T-J86414589", "T-J86414608", "T-J86414615", "T-J86414616", "T-J86414619")) #%>% filter(SID=="T-J86286945")
#view <- rawframe %>% filter (SID %in% c("T-J86258604", "T-J86286945", "T-J86286978", "T-J86324036", "T-J86324101", "T-J86353536", "T-J86353555", "T-J86353569", "T-J86353595", "T-J86353596", "T-J86353611", "T-J86353615", "T-J86353636", "T-J86353640", "T-J86414517", "T-J86414521", "T-J86414549", "T-J86414557", "T-J86414571", "T-J86414575", "T-J86414580", "T-J86414586", "T-J86414587", "T-J86414589", "T-J86414608", "T-J86414615", "T-J86414616", "T-J86414619")) %>% filter((DMW>DTO)|(DMW<DTU)) %>% left_join(SID_table)

#writexl::write_xlsx(view, './output/SFILENR_table_view_kraus.xlsx')
#rm(view)
#from_dt <- "2016-09-16 00:00:00"
#to_dt <- "2017-06-30 00:00:00"
#view <- SFILENR_table %>% filter(SARTIKELNR=='A5E02758387', DTDATUM > from_dt & DTDATUM < to_dt)
```
...
Violations for board A5E02758387 by components
```{r, echo = FALSE, results='hold', message=FALSE, warning=FALSE}
...
rawframe_sub <- rawframe %>% filter(SARTIKELNR=='A5E02758387', SID != 'T-J86414591', !is.na(viol_id)) %>% group_by (SMMNR) %>% count() %>% arrange(desc(n)) #Board T-J86414591 shows > 400 violations, 17.9.: viol_id aus group_by rausgenommen

#head(rawframe_sub,50)
rawframe_sub [1:nrow(rawframe_sub),]
...
Time distance between first fail and first pass for board "A5E02758387"
```{r, echo=FALSE, results='hide', message=FALSE, warning=FALSE}
time_diff <- SID_table %>% filter (SARTIKELNR=="A5E02758387") %>%
         select (SID, firstpass_dt, firstfail_dt, limit_viol) %>% 
         mutate(diff = (firstpass_dt - firstfail_dt)/60) %>% arrange(diff) %>% filter (!is.na(diff))

ggplot(data = time_diff, mapping = aes(x = diff)) +
         geom_histogram(binwidth = 1) +
         coord_cartesian(xlim = c(0, 60)) +
         ggtitle("Distribution of time difference 1st fail - 1st pass for A5E02758387") +
         xlab("first fail / first pass time period in min")
```
56 out of 68 boards were tested within a first fail / first pass time period of 18 minutes -> asumptions: pseudo errors;  
63 within one hour  
...
spec_bauteil <- rawframe %>% filter(SMMNR==spec_SMMNR, SARTIKELNR=="A5E02758387") %>% arrange(DTDATUM)
...
ggplot(data = spec_bauteil[spec_bauteil$SERGEBNIS=="PASS",], aes(x = DTDATUM)) +
          ...
          geom_line(aes(y = mean-6*sd), colour="gray")+
          coord_cartesian(ylim = c(spec_bauteil$DTU, spec_bauteil$DTO))+
          ggtitle (paste("trend for", spec_SMMNR, "on A5E02758387 - PASS cycles"))
...
ggplot(data = spec_bauteil[spec_bauteil$SERGEBNIS=="FAIL",], aes(x = DTDATUM)) +
          ...
          geom_line(aes(y = mean-6*sd), colour="gray")+
          coord_cartesian(ylim = c(spec_bauteil$DTU, spec_bauteil$DTO))+
          ggtitle (paste("trend for", spec_SMMNR, "on A5E02758387 - FAIL cycles"))
...
#Messwertverteilung fuer PASS
ggplot(data = spec_bauteil[spec_bauteil$SERGEBNIS=="PASS",], mapping = aes(x = DMW_norm)) +  
         geom_histogram(binwidth = 0.05) +
         coord_cartesian(xlim = c(-2, 2)) +
         ggtitle (paste("distribution for",spec_SMMNR, "on A5E02758387 - PASS cycles"))

#Boxplot fuer PASS
writexl::write_xlsx(spec_bauteil, './output/boxplot_bauteil_values.xlsx')
ggplot(data = spec_bauteil[spec_bauteil$SERGEBNIS=="PASS",], mapping = aes(y = DMW_norm)) +  
         geom_boxplot(mapping = aes(y = DMW_norm)) +
#         coord_cartesian(xlim = c(-2, 2)) +
         ggtitle (paste("distribution for", spec_SMMNR, "C1121 on A5E02758387 - PASS cycles")) +
         coord_flip()
if (plots_to_disc) ggsave ("./output/boxplot_bauteil_PASS.png")

#Messwertverteilung fuer FAIL
ggplot(data = spec_bauteil[spec_bauteil$SERGEBNIS=="FAIL",], mapping = aes(x = DMW_norm)) +  
         geom_histogram(binwidth = 0.05) +
         coord_cartesian(xlim = c(-2, 2)) +
         ggtitle (paste("distribution for",spec_SMMNR, "on A5E02758387 - FAIL cycles"))

#Boxplot fuer FAIL
ggplot(data = spec_bauteil[spec_bauteil$SERGEBNIS=="FAIL",], mapping = aes(y = DMW_norm)) +  
         geom_boxplot(mapping = aes(y = DMW_norm)) +
#         coord_cartesian(xlim = c(-2, 2)) +
         ggtitle (paste("distribution for", spec_SMMNR, "on A5E02758387 - FAIL cycles")) +
         coord_flip()
if (plots_to_disc) ggsave ("./output/boxplot_Bauteil_FAIL.png")
...
#view <- rawframe %>% filter(SCOMB_PL=="ICT_AD390650PCA5E02758387V528-K_A1")
...
# Testquery for SID # received from St. Kraus 7.9
#view <- SID_table %>% filter (SID %in% c("T-J86286978", "T-J86324036", "T-J86324101", "T-J86353536", "T-J86353555", "T-J86353569", "T-J86353595", #"T-J86353596", "T-J86353611", "T-J86353615", "T-J86353636", "T-J86353640", "T-J86414517", "T-J86414521", "T-J86414549", "T-J86414557", "T-J86414571", #"T-J86414575", "T-J86414580", "T-J86414586", "T-J86414587", "T-J86414589", "T-J86414608", "T-J86414615", "T-J86414616", "T-J86414619"))

#writexl::write_xlsx(view, './output/SID_table_view_kraus.xlsx')
#rm(view)
```
create median and standard IQR for each SCOMB_PL

```{r, echo = FALSE}
#rawframe <- rawframe %>% group_by(SCOMB_PL) %>% mutate(DMW_median=median(DMW), DMW_IQR=IQR(DMW)) %>% ungroup()

#elimiate machine name as new values median and IQR shall be independed from it (for MVP!)
rawframe <- rawframe %>% mutate(shortSCOMB_PL = substr(SCOMB_PL, gregexpr("A5E", SCOMB_PL) , nchar(SCOMB_PL))) %>% group_by(shortSCOMB_PL) %>% mutate(DMW_median=median(DMW), DMW_IQR=IQR(DMW)) %>% ungroup()
...
if (tableau_output) {
  ...
  FBG_Testlauf$Indication[FBG_Testlauf$pseudo==TRUE] <- "Pseudo Fehler"
  ...
  Messung <- rawframe %>% mutate(key_SID_DTDATUM = paste0(SID, as.character(DTDATUM))) %>%  select(key_SID_DTDATUM, NTDID, SID, DTDATUM, SMMNR, DMW, SEINHEIT, NORDER, SCOMB_PL, DTO, DTU)
  
  #calculate DMW normalized where DTU = -1.0 and DTO = 1.0
  Messung$DMW_norm <- round((Messung$DMW - Messung$DTU)/(Messung$DTO - Messung$DTU)*2 -1, digits = 8)
  
  #calculate violation level  
  Messung$DMW_viol <- "normal"
  Messung$DMW_viol[(abs(Messung$DMW_norm)>1.0) & (abs(Messung$DMW_norm)<=1.01)] <- "minimal"
  Messung$DMW_viol[(abs(Messung$DMW_norm)>1.01) & (abs(Messung$DMW_norm)<=1.05)] <- "moderat"
  Messung$DMW_viol[(abs(Messung$DMW_norm)>1.05) & (abs(Messung$DMW_norm)<=1.1)] <- "erh�ht"
  Messung$DMW_viol[abs(Messung$DMW_norm)>1.1] <- "definitiv"
 
  Messung <- select(Messung, key_SID_DTDATUM, NTDID, SID, DTDATUM, SMMNR, DMW, SEINHEIT, NORDER, SCOMB_PL, DMW_norm, DMW_viol)

  ########################################
  ### create table Messdaten_Sollwerte ###
  Messdaten_Sollwerte <- rawframe %>% select(SCOMB_PL, DTO, DTU, SEINHEIT, DMW_median, DMW_IQR) %>% distinct(SCOMB_PL, .keep_all = TRUE)
  
  Messdaten_Sollwerte <- left_join(Messdaten_Sollwerte, select(SMMNR_table, c(SCOMB_PL, X_loc, Y_loc, placementSide,SMMNR))) %>% distinct(SCOMB_PL, .keep_all = TRUE)
   
  names(Messdaten_Sollwerte)[10] <-paste("SMMNR_clean")

  #version befor Sept. 27th with SMMNR_clean
  #Messdaten_Sollwerte <- left_join(Messdaten_Sollwerte, select(SMMNR_table, c(SCOMB_PL, X_loc, Y_loc, placementSide,  SMMNR_clean))) %>% distinct(SCOMB_PL, .keep_all = TRUE)
  
  #recommendations new upper thresholds
  Messdaten_Sollwerte$DTO_001 <- Messdaten_Sollwerte$DTO + (Messdaten_Sollwerte$DTO - Messdaten_Sollwerte$DTU)/2 *0.01
  Messdaten_Sollwerte$DTO_005 <- Messdaten_Sollwerte$DTO + (Messdaten_Sollwerte$DTO - Messdaten_Sollwerte$DTU)/2 *0.05
  Messdaten_Sollwerte$DTO_010 <- Messdaten_Sollwerte$DTO + (Messdaten_Sollwerte$DTO - Messdaten_Sollwerte$DTU)/2 *0.1  
  
  #recommendations new lower thresholds
  Messdaten_Sollwerte$DTU_001 <- Messdaten_Sollwerte$DTU - (Messdaten_Sollwerte$DTO - Messdaten_Sollwerte$DTU)/2 *0.01
  Messdaten_Sollwerte$DTU_005 <- Messdaten_Sollwerte$DTU - (Messdaten_Sollwerte$DTO - Messdaten_Sollwerte$DTU)/2 *0.05
  Messdaten_Sollwerte$DTU_010 <- Messdaten_Sollwerte$DTU - (Messdaten_Sollwerte$DTO - Messdaten_Sollwerte$DTU)/2 *0.1
  
  #calculate median deviation
  Messdaten_Sollwerte$DMW_median_norm <- round(((Messdaten_Sollwerte$DMW_median - Messdaten_Sollwerte$DTU)/(Messdaten_Sollwerte$DTO - Messdaten_Sollwerte$DTU)*2-1), digits = 8)
  
  Messdaten_Sollwerte$meddev <- "normal"
  Messdaten_Sollwerte$meddev[abs(Messdaten_Sollwerte$DMW_median_norm)>0.25 & abs(Messdaten_Sollwerte$DMW_median_norm)<=0.5] <- "moderat"
  Messdaten_Sollwerte$meddev[abs(Messdaten_Sollwerte$DMW_median_norm)>0.5] <- "erh�ht"
  
  
  #calculate new thresholds because of median deviation
  Messdaten_Sollwerte$DTO_m25 <- Messdaten_Sollwerte$DTO
  Messdaten_Sollwerte$DTU_m25 <- Messdaten_Sollwerte$DTU
  
  Messdaten_Sollwerte$DMW_median_norm [is.na(Messdaten_Sollwerte$DMW_median_norm)] <- 0
  
  Messdaten_Sollwerte$DTO_m25[Messdaten_Sollwerte$DMW_median_norm > 0.25 & Messdaten_Sollwerte$meddev=="moderat"] <- Messdaten_Sollwerte$DTO[Messdaten_Sollwerte$DMW_median_norm > 0.25 & Messdaten_Sollwerte$meddev=="moderat"] + (Messdaten_Sollwerte$DTO[Messdaten_Sollwerte$DMW_median_norm > 0.25 & Messdaten_Sollwerte$meddev=="moderat"] + Messdaten_Sollwerte$DTU[Messdaten_Sollwerte$DMW_median_norm > 0.25 & Messdaten_Sollwerte$meddev=="moderat"])/2 *0.25
  
  Messdaten_Sollwerte$DTU_m25[Messdaten_Sollwerte$DMW_median_norm > 0.25 & Messdaten_Sollwerte$meddev=="moderat"] <- Messdaten_Sollwerte$DTU[Messdaten_Sollwerte$DMW_median_norm > 0.25 & Messdaten_Sollwerte$meddev=="moderat"] + (Messdaten_Sollwerte$DTO[Messdaten_Sollwerte$DMW_median_norm > 0.25 & Messdaten_Sollwerte$meddev=="moderat"] + Messdaten_Sollwerte$DTU[Messdaten_Sollwerte$DMW_median_norm > 0.25 & Messdaten_Sollwerte$meddev=="moderat"])/2 *0.25
  ...
  Messdaten_Sollwerte$DMW_IQR_norm <- round(Messdaten_Sollwerte$DMW_IQR/(Messdaten_Sollwerte$DTO - Messdaten_Sollwerte$DTU), digits = 8)
  
  Messdaten_Sollwerte$IQRdev <- "normal"
  Messdaten_Sollwerte$IQRdev[Messdaten_Sollwerte$DMW_IQR_norm>0.2 & Messdaten_Sollwerte$DMW_IQR_norm<=0.4] <- "moderat"
  Messdaten_Sollwerte$IQRdev[Messdaten_Sollwerte$DMW_IQR_norm>0.4] <- "erh�ht"
  
  #calculate new thresholds for moderate IQR range
  Messdaten_Sollwerte$DTO_i20 <- Messdaten_Sollwerte$DTO
  Messdaten_Sollwerte$DTU_i20 <- Messdaten_Sollwerte$DTU
  
  Messdaten_Sollwerte$DTO_i20[Messdaten_Sollwerte$IQRdev=="moderat"] <- Messdaten_Sollwerte$DTO[Messdaten_Sollwerte$IQRdev=="moderat"] * (1+(Messdaten_Sollwerte$DMW_IQR_norm[Messdaten_Sollwerte$IQRdev=="moderat"] - 0.2)/2)  
    
  Messdaten_Sollwerte$DTU_i20[Messdaten_Sollwerte$IQRdev=="moderat"] <- Messdaten_Sollwerte$DTU[Messdaten_Sollwerte$IQRdev=="moderat"] * (1-(Messdaten_Sollwerte$DMW_IQR_norm[Messdaten_Sollwerte$IQRdev=="moderat"] - 0.2)/2)
  
  #02.10.18 flag for recommendation
  Messdaten_Sollwerte$IQR_rcm <- ""
  # 1st hierarchy IQR
  Messdaten_Sollwerte$IQR_rcm[Messdaten_Sollwerte$IQRdev=="moderat"] <- "X"
  
  # 2nd hierarchy Median
  Messdaten_Sollwerte$med_rcm <- ""
  Messdaten_Sollwerte$med_rcm[Messdaten_Sollwerte$meddev=="moderat" & Messdaten_Sollwerte$IQRdev!="moderat"] <- "X"
  
  Messung$DMW_rcm <- ""
  Messung <- left_join(Messung, select(Messdaten_Sollwerte, c(SCOMB_PL, IQR_rcm, med_rcm)), by="SCOMB_PL")
  
  # 3rd hierarchy violatoin minimal
  Messung$DMW_rcm[Messung$DMW_viol=="minimal" & Messung$IQR_rcm!="X" & Messung$med_rcm!="X"]  <- "X"
  
  # 3rd hierarchy violatoin moderat
  Messung_interim <- Messung %>% select(SCOMB_PL, DMW_viol, DMW_rcm) %>% filter(DMW_rcm=="X") %>% distinct(SCOMB_PL, .keep_all = TRUE) %>% mutate(DMW_rcm_min = DMW_rcm)
  Messung <- left_join(Messung, select(Messung_interim, c(SCOMB_PL, DMW_rcm_min)), by="SCOMB_PL")
  Messung$DMW_rcm_min[is.na(Messung$DMW_rcm_min)] <- ""
  
  Messung$DMW_rcm[Messung$DMW_viol=="moderat" & Messung$DMW_rcm_min!="X" & Messung$IQR_rcm!="X" & Messung$med_rcm!="X"]  <- "X"
  
  # 3rd hierarchy violatoin erhoeht
  Messung_interim <- Messung %>% select(SCOMB_PL, DMW_viol, DMW_rcm) %>% filter(DMW_rcm=="X") %>% distinct(SCOMB_PL, .keep_all = TRUE) %>% mutate(DMW_rcm_mod = DMW_rcm)
  Messung <- left_join(Messung, select(Messung_interim, c(SCOMB_PL, DMW_rcm_mod)), by="SCOMB_PL")
  Messung$DMW_rcm_mod[is.na(Messung$DMW_rcm_mod)] <- ""
  
  Messung$DMW_rcm[Messung$DMW_viol=="erh�ht" & Messung$DMW_rcm_min!="X" & Messung$DMW_rcm_mod!="X" & Messung$IQR_rcm!="X" & Messung$med_rcm!="X"]  <- "X"
  Messung <- Messung %>% select(-c(IQR_rcm, med_rcm, DMW_rcm_min, DMW_rcm_mod))
  
  Messung_sub <- Messung %>% filter (DMW_rcm=="X")
  
  ##################
  ### save files ###
  
  print("... saving FBG_Testlauf table")
  outputcsv <- "./output/FBG_Testlauf.csv"
  write.table(FBG_Testlauf,   # write resulting data into CSV file with tabstops
            file=outputcsv,
            sep=",", quote=FALSE, row.names=FALSE, dec=".")
  
  print("... saving Messung table")
  outputcsv <- "./output/Messung.csv"
  write.table(Messung,   # write resulting data into CSV file with tabstops
            file=outputcsv,
            sep=",", quote=FALSE, row.names=FALSE, dec=".")
  
  print("... saving Messdaten table")
  outputcsv <- "./output/Messdaten_Sollwerte.csv"
  write.table(Messdaten_Sollwerte,   # write resulting data into CSV file with tabstops
            file=outputcsv,
            sep=",", quote=FALSE, row.names=FALSE, dec=".")
  
  rm (FBG_Testlauf, Messung, Messdaten_Sollwerte, Messung_sub)
}
```
...
```{r, echo = FALSE, message=FALSE, warning=FALSE}
rawframe_87small <- data.frame(select(rawframe87, -c(NTDID, SEINHEIT, DMW_norm, DTU, DTO, NORDER, SCOMB_PL, Ix, Ix_rel, viol_id, DMW_median, DMW_IQR, shortSCOMB_PL))) # Ausschlüsse sind spezifisch für die Messung - würden weitere features erzeugen
...
rawframe_87small <- data.frame(select(rawframe87, -c(NTDID, SEINHEIT, DMW, DTU, DTO, NORDER, SCOMB_PL, Ix, Ix_rel, viol_id, DMW_median, DMW_IQR, shortSCOMB_PL))) 
...
b) use full number of predictors for Random Forest and Lasso
...
#stratified sampling
NR <- frame87_clean %>% select(firstsecondpass_scen) %>% mutate(index = row_number()) %>% group_by(firstsecondpass_scen) %>% sample_frac(0.7)

train_index <- as.vector(NR$index)

# training and test dataset with selected predictor variables
train_n <- frame87_clean_n[train_index, ]      # Zufallsvektor definiert die Trainingsdaten
test_n <- frame87_clean_n[-train_index, ]      # ..das Komplement die Testdaten

# training and test dataset with all predictor variables for RF
train <- frame87_clean[train_index, ]          # Zufallsvektor definiert die Trainingsdaten
test  <- frame87_clean[-train_index, ]         # ..das Komplement die Testdaten

# training and test dataset with all predictor variables for Lasso
train_o <- frame87_cleanoutl[train_index, ]    # Zufallsvektor definiert die Trainingsdaten
test_o  <- frame87_cleanoutl[-train_index, ]   # ..das Komplement die Testdaten

cat("Number of Boards         :", as.character(count(distinct(frame87_clean, SID ))), "\n")

cat("Dimension dataset        :", dim(frame87_clean), "1.pass", as.character(sum(frame87_clean$firstpass_scen)), as.character(round(sum(frame87_clean$firstpass_scen)/nrow(frame87_clean)*100,3)), "% ", "1.2.pass", as.character(sum(frame87_clean$firstsecondpass_scen)), as.character(round(sum(frame87_clean$firstsecondpass_scen )/nrow(frame87_clean)*100,3)), "%", "\n")

cat("Dimension train partition:", dim(train), "1.pass", as.character(sum(train$firstpass_scen)), as.character(round(sum(train$firstpass_scen)/nrow(train)*100,3)), "% ", "1.2.pass", as.character(sum(train$firstsecondpass_scen)), as.character(round(sum(train$firstsecondpass_scen)/nrow(train)*100,3)), "%", "\n")

cat("Dimension test partition : ", dim(test), "1.pass", as.character(sum(test$firstpass_scen)), as.character(round(sum(test$firstpass_scen)/nrow(test)*100,3)), "% ", "1.2.pass", as.character(sum(test$firstsecondpass_scen)), as.character(round(sum(test$firstsecondpass_scen)/nrow(test)*100,3)), "%", "\n")
```
...
if (!file.exists(path = paste0(rel_fpath_output, "Log_mod.Rdata"))) {
  ...
  save(Log_mod, file = paste0(rel_fpath_output, "Log_mod.Rdata"))
} else {load(file = paste0(rel_fpath_output, "Log_mod.Rdata"))}  
```
```{r, echo = FALSE}
summary(Log_mod)
...
yhat_label <- as.factor(prob_to_class(yhat_prob, 0.7))
...
if (!file.exists(path = paste0(rel_fpath_output, "RF_mod.Rdata"))) {
  ...
  save(RF_mod, file = paste0(rel_fpath_output, "RF_mod.Rdata"))
} else {load(file = paste0(rel_fpath_output, "RF_mod.Rdata"))}   
...
```{r, echo = FALSE, warning=FALSE}
#train_n <- frame87_clean_n   # mal ohne split, nur model fit
if (!file.exists(path = paste0(rel_fpath_output, "Log_mod2.Rdata"))) {
  ...
  save(Log_mod2, file = paste0(rel_fpath_output, "Log_mod2.Rdata"))
} else {load(file = paste0(rel_fpath_output, "Log_mod2.Rdata"))}
```
...
# result is vector of mean values of the resprective nodes
yhat_prob <- fn_norm_zo(yhat)        # normalize predicte values
yhat_label <- as.factor(prob_to_class(yhat_prob, 0.60))
...
if (!file.exists(path = paste0(rel_fpath_output, "RF_mod2.Rdata"))) {

  RF_mod2 <- randomForest(x=select(train, -c(NTHID, SFILENAME, DTINSERT, SID, SARTIKELNR, DTDATUM, SPLATZNR, SERGEBNIS,
                                             BEFUND_clean, pseudo, firstpass_scen, secondpass_scen, firstsecondpass_scen, xpass)),
                          y=as.factor(train$firstsecondpass_scen), cutoff=c(0.3,0.7), importance=TRUE, ntree=500)

  save(RF_mod2, file = paste0(rel_fpath_output, "RF_mod2.Rdata"))
} else {load(file = paste0(rel_fpath_output, "RF_mod2.Rdata"))}
...
# get sample tree
#randomForest::getTree(RF_mod2, k = 300, labelVar = TRUE)

varImpPlot(RF_mod2, type=1,main = "Variable Importance")
varImpPlot(RF_mod2, type=2,main = "Variable Importance")

top_RF_fs2 <- as.data.frame(RF_mod2$importance) %>% mutate (Component = row.names(as.data.frame(RF_mod2$importance))) %>% select(-c("0","1"))  %>% arrange(desc(MeanDecreaseAccuracy))# %>% select(-c("MeanDecreaseAccuracy","MeanDecreaseGini"))
top_RF_fs2[1:30,]
```
...
library(e1071)
# Evaluation with test data set
yhat <- predict(RF_mod2, select(test, -c(NTHID, SFILENAME, DTINSERT, SID, SARTIKELNR, DTDATUM, SPLATZNR, SERGEBNIS, BEFUND_clean, pseudo, firstpass_scen, secondpass_scen, firstsecondpass_scen, xpass)), type="prob")[,"1"] # take secend column of probabilities

# result is vector of mean values of the resprective nodes
yhat_prob <- fn_norm_zo(yhat)        # normalize predicte values
yhat_label <- as.factor(prob_to_class(yhat_prob, 0.6))
...
# Lift / ROC Chart for test data
par(mfrow=c(1,2))
Lift_Chart (yhat_prob, ytrue, "Lift Random Forest", "black")
ROC_Chart (yhat_prob, ytrue, "ROC Random Forest")
```
location of predictors on the board (visual clustering)

```{r, echo = FALSE, message=FALSE, warning=FALSE}
SMMNR_subtable <- SMMNR_table %>% filter(SARTIKELNR=="A5E02758387") %>% filter (SMMNR %in% top_RF_fs2$Component [1:30]) %>% group_by(SMMNR) %>% summarise(count_viol = sum(count_viol, na.rm=TRUE), X_loc = unique(X_loc), Y_loc = unique(Y_loc), SMMNR_clean = unique(SMMNR_clean), placementSide = unique(placementSide))

ggplot(data = filter(SMMNR_subtable, placementSide=="A"), mapping = aes(x = X_loc, y = Y_loc, colour = count_viol), size=1.0) +
    geom_point() +
    geom_text (aes(label=SMMNR_clean), hjust=0.5, vjust=-0.5, size=3) +
    coord_cartesian(xlim = unlist(c(loc_mm[loc_mm$SARTIKELNR=="A5E02758387", 2], loc_mm[loc_mm$SARTIKELNR=="A5E02758387", 4]))) +
    coord_cartesian(ylim = unlist(c(loc_mm[loc_mm$SARTIKELNR=="A5E02758387", 3], loc_mm[loc_mm$SARTIKELNR=="A5E02758387", 5]))) +
    ggtitle("Localisation Info for board A5E02758387 for RF side A")
if (plots_to_disc) ggsave ("./output/location_RF_A.png")

ggplot(data = filter(SMMNR_subtable, placementSide=="B"), mapping = aes(x = X_loc, y = Y_loc, colour = count_viol), size=1.0) +
    geom_point() +
    geom_text (aes(label=SMMNR_clean), hjust=0.5, vjust=-0.5, size=3) +
    coord_cartesian(xlim = unlist(c(loc_mm[loc_mm$SARTIKELNR=="A5E02758387", 2], loc_mm[loc_mm$SARTIKELNR=="A5E02758387", 4]))) +
    coord_cartesian(ylim = unlist(c(loc_mm[loc_mm$SARTIKELNR=="A5E02758387", 3], loc_mm[loc_mm$SARTIKELNR=="A5E02758387", 5]))) +
    ggtitle("Localisation Info for board A5E02758387 for RF side B")
if (plots_to_disc) ggsave ("./output/location_RF_B.png")
```

visualized tree example
```{r, echo = FALSE, message=FALSE, warning=FALSE}
library(dplyr)
library(ggraph)
library(igraph)
...
RF 10 fold crosss validation
```{r, echo = FALSE}
library(caret)
library(doSNOW)

if (!file.exists(path = paste0(rel_fpath_output, "cv.10.RF_mod.Rdata"))) {  
  set.seed(123456789)
  # stratification
  cv.10.folds <- createMultiFolds(train$firstsecondpass_scen, k = 10, times = 10)
  # setup a caret's training control object
  ctrl.1 <- trainControl(method = "repeatedcv", number = 10, repeats = 1,
                       index = cv.10.folds)
  #setup doSNOW
  cl <- makeCluster(6, type = "SOCK")
  registerDoSNOW(cl)

  cv.10.RF_mod <- train(select(train, -c(NTHID, SFILENAME, DTINSERT, SID, SARTIKELNR, DTDATUM, SPLATZNR, SERGEBNIS,
                                    BEFUND_clean, pseudo, firstpass_scen, secondpass_scen, firstsecondpass_scen, xpass)),
                   as.factor(train$firstsecondpass_scen), method = "rf", tuneLength = 15, ntree = 400, trcontrol = ctrl.1)

  stopCluster(cl)
  save(cv.10.RF_mod, file = paste0(rel_fpath_output , "cv.10.RF_mod.Rdata"))
} else {load(file = paste0(rel_fpath_output, "cv.10.RF_mod.Rdata"))}  
```
...
#summary(cv.10.RF_mod)
print(varImp(cv.10.RF_mod), top = 30)
plot(varImp(cv.10.RF_mod), top = 30)
   
library(e1071)
# Evaluation with test data set
yhat <- predict(cv.10.RF_mod, select(test, -c(NTHID, SFILENAME, DTINSERT, SID, SARTIKELNR, DTDATUM, SPLATZNR, SERGEBNIS, BEFUND_clean, pseudo, firstpass_scen, secondpass_scen, firstsecondpass_scen, xpass)), type="prob")[,"1"] # take secend column of probabilities
...
# Lift / ROC Chart for test data
par(mfrow=c(1,2))
Lift_Chart (yhat_prob, ytrue, "Lift Random Forest 10-f", "black")
ROC_Chart (yhat_prob, ytrue, "ROC Random Forest 10-f")
```
  
RF 5 fold crosss validation
```{r, echo = FALSE}
if (!file.exists(path = paste0(rel_fpath_output, "cv.5.RF_mod.Rdata"))) {  
  set.seed(123456789)
  # stratification
  cv.5.folds <- createMultiFolds(train$firstsecondpass_scen, k = 5, times = 10)
  # setup a caret's training control object
  ctrl.2 <- trainControl(method = "repeatedcv", number = 5, repeats = 1,
                       index = cv.5.folds)
  #setup doSNOW
  cl <- makeCluster(6, type = "SOCK")
  registerDoSNOW(cl)

  cv.5.RF_mod <- train(select(train, -c(NTHID, SFILENAME, DTINSERT, SID, SARTIKELNR, DTDATUM, SPLATZNR, SERGEBNIS,
                                    BEFUND_clean, pseudo, firstpass_scen, secondpass_scen, firstsecondpass_scen, xpass)),
                   as.factor(train$firstsecondpass_scen), method = "rf", tuneLength = 15, ntree = 400, trcontrol = ctrl.2 )

  stopCluster(cl)
  save(cv.5.RF_mod, file = paste0(rel_fpath_output , "cv.5.RF_mod.Rdata"))
} else {load(file = paste0(rel_fpath_output, "cv.5.RF_mod.Rdata"))} 
```
...
#summary(cv.5.RF_mod)
print(varImp(cv.5.RF_mod), top = 30)
plot(varImp(cv.5.RF_mod), top = 30)

library(e1071)
# Evaluation with test data set
yhat <- predict(cv.5.RF_mod, select(test, -c(NTHID, SFILENAME, DTINSERT, SID, SARTIKELNR, DTDATUM, SPLATZNR, SERGEBNIS, BEFUND_clean, pseudo, firstpass_scen, secondpass_scen, firstsecondpass_scen, xpass)), type="prob")[,"1"] # take secend column of probabilities
...
# Lift / ROC Chart for test data
par(mfrow=c(1,2))
Lift_Chart (yhat_prob, ytrue, "Lift Random Forest 5-f", "black")
ROC_Chart (yhat_prob, ytrue, "ROC Random Forest 5-f")
```
  
RF 3 fold crosss validation
```{r, echo = FALSE}
if (!file.exists(path = paste0(rel_fpath_output, "cv.3.RF_mod.Rdata"))) {  
  set.seed(123456789)
  # stratification
  cv.3.folds <- createMultiFolds(train$firstsecondpass_scen, k = 3, times = 10)
  # setup a caret's training control object
  ctrl.3 <- trainControl(method = "repeatedcv", number = 3, repeats = 1,
                       index = cv.3.folds)
  #setup doSNOW
  cl <- makeCluster(6, type = "SOCK")
  registerDoSNOW(cl)

  cv.3.RF_mod <- train(select(train, -c(NTHID, SFILENAME, DTINSERT, SID, SARTIKELNR, DTDATUM, SPLATZNR, SERGEBNIS,
                                    BEFUND_clean, pseudo, firstpass_scen, secondpass_scen, firstsecondpass_scen, xpass)),
                   as.factor(train$firstsecondpass_scen), method = "rf", tuneLength = 15, ntree = 400, trcontrol = ctrl.3 )

  stopCluster(cl)
  save(cv.3.RF_mod, file = paste0(rel_fpath_output , "cv.3.RF_mod.Rdata"))
} else {load(file = paste0(rel_fpath_output, "cv.3.RF_mod.Rdata"))} 
```
...
#summary(cv.3.RF_mod)
print(varImp(cv.3.RF_mod), top = 30)
plot(varImp(cv.3.RF_mod), top = 30)
   
library(e1071)
# Evaluation with test data set
yhat <- predict(cv.3.RF_mod, select(test, -c(NTHID, SFILENAME, DTINSERT, SID, SARTIKELNR, DTDATUM, SPLATZNR, SERGEBNIS, BEFUND_clean, pseudo, firstpass_scen, secondpass_scen, firstsecondpass_scen, xpass)), type="prob")[,"1"] # take secend column of probabilities
...
# Lift / ROC Chart for test data
par(mfrow=c(1,2))
Lift_Chart (yhat_prob, ytrue, "Lift Random Forest 3-f", "black")
ROC_Chart (yhat_prob, ytrue, "ROC Random Forest 3-f")
```
...
all variables         | 123456789|   no         | 0.30   | 98,49% (5 false positive)
                      |          |              |        |  
all variables         | 123456789|   no         | 0.60   | 98,06% (7 false positive) 
3fold cv              |          |              |        |  
...
Reasoning for model seletion:  
* performs both variable selection and regularization in order to enhance the prediction accuracy and interpretability
* tries to avoid overfitting penalizing large coefficients
* can shrink some of the coefficients to exactly zero
...
if (!file.exists(path = paste0(rel_fpath_output, "cv.lasso_mod.Rdata"))) {  
  cl <- makeCluster(4, type = "SOCK")
  registerDoSNOW(cl)
  ## Lasso
  ## Note alpha=1 for lasso only and can blend with ridge penalty down to alpha=0 ridge only.
  
  cv.lasso_mod <- cv.glmnet(as.matrix(select(train_o, -c(NTHID, SFILENAME, DTINSERT, SID, SARTIKELNR, DTDATUM, SPLATZNR, SERGEBNIS,
                                                         BEFUND_clean, pseudo, firstpass_scen, secondpass_scen, firstsecondpass_scen, xpass))),
                            train_o$firstsecondpass_scen, alpha=1, parallel=TRUE, standardize=TRUE, type.measure='auc')
  
  save(cv.lasso_mod, file = paste0(rel_fpath_output , "cv.lasso_mod.Rdata"))
} else {load(file = paste0(rel_fpath_output, "cv.lasso_mod.Rdata"))}

library(e1071)
# Evaluation with test data set
yhat <- predict(cv.lasso_mod, as.matrix(select(test_o, -c(NTHID, SFILENAME, DTINSERT, SID, SARTIKELNR, DTDATUM, SPLATZNR, SERGEBNIS, BEFUND_clean, pseudo, firstpass_scen, secondpass_scen, firstsecondpass_scen, xpass))), type = "class", s = "lambda.min")
yhat_prob <- fn_norm_zo(yhat)        # normalize predicte values
yhat_label <- as.factor(prob_to_class(yhat_prob, 0.5))

# Confusion matrix
ytrue <- test_o$firstsecondpass_scen
caret::confusionMatrix(yhat_label, as.factor(ytrue), positive="1")

# Lift / ROC Chart for test data
par(mfrow=c(1,2))
Lift_Chart (as.vector(yhat_prob), ytrue, "Lift Lasso", "black")
ROC_Chart (yhat_prob, ytrue, "ROC Lasso")
```

adaptive Lasso
```{r, echo = FALSE, message=FALSE, warning=FALSE}
#* Adaptive Lasso, as a regularization method, avoids overfitting penalizing large coefficients
...
cv.ridge <- cv.glmnet(as.matrix(select(train_o, -c(NTHID, SFILENAME, DTINSERT, SID, SARTIKELNR, DTDATUM, SPLATZNR, SERGEBNIS, BEFUND_clean, pseudo, firstpass_scen, secondpass_scen, firstsecondpass_scen, xpass))), train_o$firstsecondpass_scen,  alpha=0, parallel=TRUE, standardize=TRUE)
  
w3 <- 1/abs(matrix(coef(cv.ridge, s=cv.ridge$lambda.min)[, 1][2:(ncol(select(train_o, -c(NTHID, SFILENAME, DTINSERT, SID, SARTIKELNR, DTDATUM, SPLATZNR, SERGEBNIS, BEFUND_clean, pseudo, firstpass_scen, secondpass_scen, firstsecondpass_scen, xpass)))+1)] ))^1 ## Using gamma = 1
  w3[w3[,1] == Inf] <- 999999999 ## Replacing values estimated as Infinite for 999999999

## Adaptive Lasso
if (!file.exists(path = paste0(rel_fpath_output, "cv.10.a_lasso_mod.Rdata"))) {
    
  cv.10.a_lasso_mod <- cv.glmnet(as.matrix(select(train_o, -c(NTHID, SFILENAME, DTINSERT, SID, SARTIKELNR, DTDATUM, SPLATZNR, SERGEBNIS, BEFUND_clean, pseudo, firstpass_scen, secondpass_scen, firstsecondpass_scen, xpass))), train_o$firstsecondpass_scen, alpha=1, parallel=TRUE, standardize=TRUE, type.measure='auc', penalty.factor=w3, nfolds = 10)
    
  save(cv.10.a_lasso_mod, file = paste0(rel_fpath_output , "cv.10.a_lasso_mod.Rdata"))
} else {load(file = paste0(rel_fpath_output, "cv.10.a_lasso_mod.Rdata"))}    
  
if (!file.exists(path = paste0(rel_fpath_output, "cv.5.a_lasso_mod.Rdata"))) {
  cv.5.a_lasso_mod <- cv.glmnet(as.matrix(select(train_o, -c(NTHID, SFILENAME, DTINSERT, SID, SARTIKELNR, DTDATUM, SPLATZNR, SERGEBNIS, BEFUND_clean, pseudo, firstpass_scen, secondpass_scen, firstsecondpass_scen, xpass))), train_o$firstsecondpass_scen, alpha=1, parallel=TRUE, standardize=TRUE, type.measure='auc', penalty.factor=w3, nfolds = 5)
    
  save(cv.5.a_lasso_mod, file = paste0(rel_fpath_output , "cv.5.a_lasso_mod.Rdata"))    
} else {load(file = paste0(rel_fpath_output, "cv.5.a_lasso_mod.Rdata"))}  
  
if (!file.exists(path = paste0(rel_fpath_output, "cv.3.a_lasso_mod.Rdata"))) {
  cv.3.a_lasso_mod <- cv.glmnet(as.matrix(select(train_o, -c(NTHID, SFILENAME, DTINSERT, SID, SARTIKELNR, DTDATUM, SPLATZNR, SERGEBNIS, BEFUND_clean, pseudo, firstpass_scen, secondpass_scen, firstsecondpass_scen, xpass))), train_o$firstsecondpass_scen, alpha=1, parallel=TRUE, standardize=TRUE, type.measure='auc', penalty.factor=w3, nfolds = 3)
    
  save(cv.3.a_lasso_mod, file = paste0(rel_fpath_output , "cv.3.a_lasso_mod.Rdata"))
} else {load(file = paste0(rel_fpath_output, "cv.3.a_lasso_mod.Rdata"))}
...
```{r, echo = FALSE, message=FALSE, warning=FALSE}
#coef(cv.lasso_mod, s=cv.lasso_mod$lambda.min)
cat("Minimum lamda no cv     :", cv.lasso_mod$lambda.min, "\n")
plot(cv.lasso_mod, xvar="lambda", label=TRUE)
title("Lasso no cv", line = -1)
cat("Minimum lamda 10 fold cv:", cv.10.a_lasso_mod$lambda.min, "\n")
plot(cv.10.a_lasso_mod, xvar="lambda", label=TRUE)
title("Lasso cv 10", line = -1)
cat("Minimum lamda  5 fold cv:", cv.5.a_lasso_mod$lambda.min, "\n")
plot(cv.5.a_lasso_mod, xvar="lambda", label=TRUE)
title("Lasso cv 5", line = -1)
cat("Minimum lamda  3 fold cv:", cv.3.a_lasso_mod$lambda.min, "\n")
plot(cv.3.a_lasso_mod, xvar="lambda", label=TRUE)
title("Lasso cv 3", line = -1)
```
top 30 postive / negative coeffients by magnitude
model used: no fold cv

```{r, echo = FALSE, message=FALSE, warning=FALSE}
top_Lasso_coeff <- coef(cv.lasso_mod, s=cv.lasso_mod$lambda.min)
top_Lasso_coeff <- cbind.data.frame(unlist(top_Lasso_coeff@Dimnames[1])[top_Lasso_coeff@i+1], top_Lasso_coeff@x, exp(top_Lasso_coeff@x))
names(top_Lasso_coeff) <- c("SMMNR", "Coefficient", "Odds")
top_Lasso_coeff <- top_Lasso_coeff %>% arrange(desc(Coefficient))
cat("Number of coefficients:", nrow(top_Lasso_coeff))
top_Lasso_coeff[1:30,] # top 30 positive coefficients
top_Lasso_coeff[(nrow(top_Lasso_coeff)-30):nrow(top_Lasso_coeff),]  # top 30 negative coefficients
```
...
Feature Selection     | Seed     | Oversampling | Cutoff | Accuracy  
----------------------|----------|--------------|--------|--------------------------
all variables         | 123      |   no         | 0.55   | 98,28% (4 false positive) 
standardization=TRUE  |          |              |        |    
                      |          |              |        |  
all variables         | 123456789|   no         | 0.60   | 98,92% (2 false positive)
standardization=TRUE  |          |              |        |      
                      |          |              |        | 
all variables         | 123      |   no         | 0.50   | 98,71% (4 false positive)
                      |          |              |        |  
all variables         | 123456789|   no         | 0.55   | 98,28% (4 false positive)
                      |          |              |        |                      
excl. highly          |          |              |        |  
correlated variables  | 123      |   no         | 0.55   | 98,28% (3 false positive) 
                      |          |              |        |                          
excl. highly          | 123456789|   no         | 0.55   | 97,42% (4 false positive)
correlated variables  |          |              |        |  
                      |          |              |        |  
```{r}

```
model evaluation for Adaptive Lasso regression  
...
standardization=TRUE  |          |              |        |    
                      |          |              |        |  
all variables         | 123456789|   no         | 0.60   | 98,92% (4 false positive)
standardization=TRUE  |          |              |        |  
                      |          |              |        |  
all variables         | 123      |   no         | 0.55   | 98,06% (4 false positive)
                      |          |              |        |  
all variables         | 123456789|   no         | 0.55   | 98,06% (4 false positive)
                      |          |              |        | 
5 fold cv             | 123456789|   no         | 0.60   | 99,14% (2 false positive)           

Adaptive Lasso reduces the number of coeffiencts to less than 100-200 whereas Lasso comes up with about 205 coeffiencts

10 fold cross validation
```{r, echo = FALSE, message=FALSE, warning=FALSE}
library(e1071)
# Evaluation with test data set
yhat <- predict(cv.10.a_lasso_mod, as.matrix(select(test_o, -c(NTHID, SFILENAME, DTINSERT, SID, SARTIKELNR, DTDATUM, SPLATZNR, SERGEBNIS, BEFUND_clean, pseudo, firstpass_scen, secondpass_scen, firstsecondpass_scen, xpass))), type = "class", s = "lambda.min")
yhat_prob <- fn_norm_zo(yhat)        # normalize predicte values
yhat_label <- as.factor(prob_to_class(yhat_prob, 0.5))
...
# Lift / ROC Chart for test data
par(mfrow=c(1,2))
Lift_Chart (as.vector(yhat_prob), ytrue, "Lift Lasso 10-f", "black")
ROC_Chart (yhat_prob, ytrue, "ROC Lasso 10-f")
```

Adaptive Lasso 5 fold cross validation
```{r, echo = FALSE, message=FALSE, warning=FALSE}
library(e1071)
# Evaluation with test data set
yhat <- predict(cv.5.a_lasso_mod, as.matrix(select(test_o, -c(NTHID, SFILENAME, DTINSERT, SID, SARTIKELNR, DTDATUM, SPLATZNR, SERGEBNIS, BEFUND_clean, pseudo, firstpass_scen, secondpass_scen, firstsecondpass_scen, xpass))), type = "class", s = "lambda.min")
yhat_prob <- fn_norm_zo(yhat)        # normalize predicte values
yhat_label <- as.factor(prob_to_class(yhat_prob, 0.5))
...
# Lift / ROC Chart for test data
par(mfrow=c(1,2))
Lift_Chart (as.vector(yhat_prob), ytrue, "Lift Lasso 5-f", "black")
ROC_Chart (yhat_prob, ytrue, "ROC Lasso 5-f")
```
...
library(e1071)
# Evaluation with test data set
yhat <- predict(cv.3.a_lasso_mod, as.matrix(select(test_o, -c(NTHID, SFILENAME, DTINSERT, SID, SARTIKELNR, DTDATUM, SPLATZNR, SERGEBNIS, BEFUND_clean, pseudo, firstpass_scen, secondpass_scen, firstsecondpass_scen, xpass))), type = "class", s = "lambda.min")
yhat_prob <- fn_norm_zo(yhat)        # normalize predicte values
yhat_label <- as.factor(prob_to_class(yhat_prob, 0.5))
...
# Lift / ROC Chart for test data
par(mfrow=c(1,2))
Lift_Chart (as.vector(yhat_prob), ytrue, "Lift Lasso 3-f", "black")
ROC_Chart (yhat_prob, ytrue, "ROC Lasso 3-f")
```

location of predictors on the board (visual clustering)
```{r, echo = FALSE, message=FALSE, warning=FALSE}
SMMNR_subtable <- SMMNR_table %>% filter(SARTIKELNR=="A5E02758387") %>% filter (SMMNR %in% top_Lasso_coeff$SMMNR[1:15] | SMMNR %in% top_Lasso_coeff$SMMNR [(nrow(top_Lasso_coeff)-15):nrow(top_Lasso_coeff)]) %>% group_by(SMMNR) %>% summarise(count_viol = sum(count_viol, na.rm=TRUE), X_loc = unique(X_loc), Y_loc = unique(Y_loc), SMMNR_clean = unique(SMMNR_clean), placementSide = unique(placementSide))
...
ggplot(data = filter(SMMNR_subtable, placementSide=="A"), mapping = aes(x = X_loc, y = Y_loc, colour = count_viol), size=1.0) +
    ...
    coord_cartesian(xlim = unlist(c(loc_mm[loc_mm$SARTIKELNR=="A5E02758387", 2], loc_mm[loc_mm$SARTIKELNR=="A5E02758387", 4]))) +
    coord_cartesian(ylim = unlist(c(loc_mm[loc_mm$SARTIKELNR=="A5E02758387", 3], loc_mm[loc_mm$SARTIKELNR=="A5E02758387", 5]))) +
    ggtitle("Localisation Info for board A5E02758387 for Lasso side A")
if (plots_to_disc) ggsave ("./output/location_Lasso_A.png")
...
ggplot(data = filter(SMMNR_subtable, placementSide=="B"), mapping = aes(x = X_loc, y = Y_loc, colour = count_viol), size=1.0) +
    ...
    coord_cartesian(xlim = unlist(c(loc_mm[loc_mm$SARTIKELNR=="A5E02758387", 2], loc_mm[loc_mm$SARTIKELNR=="A5E02758387", 4]))) +
    coord_cartesian(ylim = unlist(c(loc_mm[loc_mm$SARTIKELNR=="A5E02758387", 3], loc_mm[loc_mm$SARTIKELNR=="A5E02758387", 5]))) +
    ggtitle("Localisation Info for board A5E02758387 for Lasso side B")
if (plots_to_disc) ggsave ("./output/location_Lasso_B.png")
```

RPART (not recommended)
```{r, echo = FALSE, message=FALSE, warning=FALSE}
library(rpart)
library(rpart.plot)
rp_mod <- rpart(as.factor(firstsecondpass_scen) ~.,  data = select(train, -c(NTHID, SFILENAME, DTINSERT, SID, SARTIKELNR, DTDATUM, SPLATZNR, SERGEBNIS, BEFUND_clean, pseudo, firstpass_scen, secondpass_scen, xpass)))
rpart.plot(rp_mod)
```
...
library(e1071)
# Evaluation with test data set
yhat_label <- rpart.predict(rp_mod, select(test, -c(NTHID, SFILENAME, DTINSERT, SID, SARTIKELNR, DTDATUM, SPLATZNR, SERGEBNIS, BEFUND_clean, pseudo, firstpass_scen, secondpass_scen, xpass)), type = "class")
#yhat_prob <- fn_norm_zo(yhat)        # normalize predicte values
...
# 5.2.4 Grandient Boosted Tree

```{r, echo = FALSE, message=FALSE, warning=FALSE}
library(gbm)
library(doSNOW)

if (!file.exists(path = paste0(rel_fpath_output, "cv.5.gbt_mod.Rdata"))) {
  ...
  #cv.5.folds <- createMultiFolds(train$firstsecondpass_scen, k = 5, times = 10)
  # setup a caret's training control object
  ctrl.2 <- trainControl(method = "repeatedcv", number = 3, repeats = 1)#,
                      # index = cv.5.folds)
  #setup doSNOW
  #cl <- makeCluster(6, type = "SOCK")
  #registerDoSNOW(cl)
  #registerDoSEQ()
  x <- select(train_n, -c(NTHID, SFILENAME, DTINSERT, SID, SARTIKELNR, DTDATUM, SPLATZNR, SERGEBNIS,
                                    BEFUND_clean, pseudo, firstpass_scen, secondpass_scen, firstsecondpass_scen, xpass))
    
  cv.5.gbt_mod <- train(x,
                   as.factor(train_n$firstsecondpass_scen), method = "gbm", distribution = "gaussian", n.trees = 500)#, trcontrol = ctrl.2)
...
#                   as.factor(train$firstsecondpass_scen), method = "rf", tuneLength = 15, ntree = 400, trcontrol = ctrl.1)
  ...
  save(cv.5.gbt_mod, file = paste0(rel_fpath_output , "cv.5.gbt_mod.Rdata"))
} else {load(file = paste0(rel_fpath_output, "cv.5.gbt_mod.Rdata"))}  
...
# Lift / ROC Chart for test data
par(mfrow=c(1,2))
Lift_Chart (as.vector(yhat_prob), ytrue, "Lift Gradient Boosting", "black")
ROC_Chart (yhat_prob, ytrue, "ROC Gradient Boosting")
```